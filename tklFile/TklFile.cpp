...
namespace nsK2EngineLow {
	void TklFile::Load(const char* filepath)
	{
		...
		for (int i = 0; i < m_objectCout; i++) {
			...
			m_objectlist[i].charData.resize(numChar);
			//charDataの処理
			for (int j = 0; j < numChar; j++) {
				size_t stringSize = 0;
				fread(&stringSize, 1, 1, file);
				//ユニークポインタを作成。
				m_objectlist[i].charData[j] = std::make_unique<char[]>(stringSize+1);
				fread(m_objectlist[i].charData[j].get(), stringSize+1, 1, file);
			}
			int numVector3 = 0;
			fread(&numVector3, sizeof(numVector3), 1, file);
			//Vector3Dataの処理
			for (int j = 0; j < numVector3; j++) {
				float x, y, z;
